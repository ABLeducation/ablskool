...
class user_profile_teacher(models.Model):
    ...
    def get_escalation(self):
        if self.user.email in ['training1@thinnkware.com', 'training2@thinnkware.com','training10@thinnkware.com',
                               'training8@thinnkware.com','training5@thinnkware.com']:
            return 1
        else:
            return 2
...
class Enquiry(models.Model):
    name=models.CharField(max_length=50)
    contact=models.CharField(max_length=100,validators=[validate_mobile_number])
...
class UserActivity(models.Model):
    ...
    def get_total_time_spent(user):
        result = UserActivity.objects.filter(user=user, duration__isnull=False).aggregate(total_time=Sum('duration'))
        return result['total_time'] if result['total_time'] else timedelta(0)
